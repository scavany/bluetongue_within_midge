...
NLL <- function(pars) {
    ...
    with(as.list(pars),{
        beta.s <- as.numeric(exp(lbeta))
        p.s <- as.numeric(exp(lp))
        c.s <- as.numeric(exp(lc))
        k <- as.numeric(plogis(lk))
        tau <- as.numeric(exp(ltau))
        b.s <- as.numeric(exp(lb))
        mu.s <- as.numeric(exp(lmu))
        mu.m <- mu.s; p.m <- p.s; beta.m <- beta.s; c.m <- c.s; b.m <- b.s
        parameters = c(k=k,tau=tau,p.m=p.m,p.s=p.s,c.m=c.m,c.s=c.s,b.m=b.m,b.s=b.s,
                       beta.m=beta.m,beta.s=beta.s,mu.m=mu.m,mu.s=mu.s)
        state.names <- c("V.m","T.m","I.m","V.h","T.s","I.s","N.s")

        ## Intrathoracic infections
        V.h <- fu.intrathoracic$titre[1]; T.s <- tau*V.h
        state = c(0,0,0,V.h,T.s,0,T.s)
        names(state) <- state.names
        dat = ode(y = state, times = times, func = wv.BTV.test, parms = parameters)
        titre.model.it <- dat[data.hours.it,"V.h"]

        ## Then full infection, accounting for both succesful and failed infections
        V.m <- initial.titre; T.m <- initial.titre*tau;
        state = c(V.m,T.m,0,0,0,0,0)
        names(state) <- state.names
        dat = ode(y = state, times = times, func = wv.BTV.test, parms = parameters)
        titre.disseminated <- rowSums(dat[data.hours,c("V.m","V.h")])
        parameters["k"] <- 1
        dat = ode(y = state, times = times, func = wv.BTV.test, parms = parameters)
        titre.failure <- dat[data.hours,"V.m"]
...
LL <- function(par) {
    pars <- rep(0,length(par))
    pars[2:length(par)] <- log(par[2:length(par)])
    pars[1] <- qlogis(par[1])
    names(pars) <- names(pars.init)
    -NLL(pars)
}

pars.init <- c(lk=qlogis(0.01),ltau=log(0.1),lp=log(1.5e3),lc=log(10),lb=log(1e-3),
               lbeta=log(1e-7),lmu=log(0.5))
optim.out <- optim(pars.init,NLL,control=list(maxit=1e3))
