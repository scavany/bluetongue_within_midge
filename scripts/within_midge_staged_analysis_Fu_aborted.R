...
NLL <- function(pars) {
    ...
    with(as.list(pars),{
        ...
        c.s <- as.numeric(exp(lc.s))#p.s / geometric.mean(tail(fu.intrathoracic$titre))
        mu.s <- as.numeric(exp(lmu.s))
        parameters = c(c.s = c.s, beta.s = beta.s, p.s = p.s,mu.s = mu.s)
        ...
        mu.m <- as.numeric(exp(lmu.m))
        prop.s <- 1/(1+exp(-as.numeric(exp(lprop.s))))
        epsilon <- 1 + 2/(1+exp(-as.numeric(exp(lepsilon))))
        ...
        parameters = c(c.m = c.m, c.s = c.s, beta.m = beta.m, beta.s = beta.s, n = n,
                       p.m = p.m, p.s = p.s, epsilon = epsilon, prop.s = prop.s,
                       mu.m = mu.m, mu.s = mu.s)
...
pars.init <- c(lbeta.m=0,lp.m=0,lc.m=0,lepsilon=0,lmu.m=0,lprop.s=0,
               lbeta.s=0,lp.s=0,lc.s=0,lmu.s=0)
optim.out <- optim(pars.init,NLL,control=list(maxit=1000))
...
mu.s <- as.numeric(exp(optim.out$par["lmu.s"]))
...
epsilon <- 1 + 2 / (1 + exp(-as.numeric(exp(optim.out$par["lepsilon"]))))
beta.m <- as.numeric(exp(optim.out$par["lbeta.m"]))
p.m <- as.numeric(exp(optim.out$par["lp.m"]))
c.m <- as.numeric(exp(optim.out$par["lc.m"]))
mu.m <- as.numeric(exp(optim.out$par["lmu.m"]))
prop.s <- 1/(1 + exp(-as.numeric(exp(optim.out$par["lprop.s"]))))
inoculum.factor <- 1
parameters = c(c.m = c.m, c.s = c.s, beta.m = beta.m, beta.s = beta.s, n = n,
               p.m = p.m, p.s = p.s, epsilon = epsilon, prop.s = prop.s,
               mu.m = mu.m, mu.s = mu.s)
